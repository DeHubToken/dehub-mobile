...
type UseProviderLifecycleReturn = {
  ...
  adoptProvider: (prov: EIP1193Provider | null, opts?: { setReady?: boolean; mode?: "local" | "override" | "auth" }) => Promise<void>;
...
export function useProviderLifecycle({
  ...
  useEffect(() => {
    return () => {
      ...
      if (providerMetaRef.current.listenerCleanup) {
        try { providerMetaRef.current.listenerCleanup(); } catch {}
      }
      if (providerMetaRef.current.healthInterval) clearInterval(providerMetaRef.current.healthInterval);
      if (providerMetaRef.current.scheduledValidateTimeout) clearTimeout(providerMetaRef.current.scheduledValidateTimeout);
    };
  }, []);

  const parseChainId = useCallback((raw: any): number | undefined => {
    try {
      const parsed = typeof raw === "string" && raw?.startsWith?.("0x") ? parseInt(raw, 16) : Number(raw);
  ...
  const fetchAccounts = useCallback(async (prov: EIP1193Provider | null): Promise<string[] | null> => {
    if (!prov?.request) return null;
    try {
      const accounts: unknown = await prov.request({ method: "eth_accounts" });
  ...
  const attachProviderEventListeners = useCallback((prov: EIP1193Provider | null) => {
    if (!prov) return () => {};
    const onChainChanged = (next: any) => {
      const parsed = typeof next === "string" && next.startsWith("0x") ? parseInt(next, 16) : Number(next);
      if (!Number.isNaN(parsed)) setChainId(parsed);
    };
    const onAccountsChanged = (accs: string[]) => {
      if (!accs || accs.length === 0) scheduleValidation("accountsChanged-empty");
    };
    const onDisconnect = (err: any) => {
      log.warn("provider:event:disconnect", err?.message || err);
      scheduleValidation("disconnect");
    };
    try { prov.on?.("chainChanged", onChainChanged); } catch {}
    try { prov.on?.("accountsChanged", onAccountsChanged); } catch {}
    try { prov.on?.("disconnect", onDisconnect); } catch {}
    return () => {
      try { prov.removeListener?.("chainChanged", onChainChanged); } catch {}
      try { prov.removeListener?.("accountsChanged", onAccountsChanged); } catch {}
      try { prov.removeListener?.("disconnect", onDisconnect); } catch {}
    };
  }, [log]);

  const adoptProvider = useCallback(async (prov: EIP1193Provider | null, opts?: { setReady?: boolean; mode?: "local" | "override" | "auth" }) => {
    if (!prov) return;
    try {
      let raw: any;
      if (typeof prov?.request === "function") raw = await prov.request({ method: "eth_chainId" });
    ...
    providerMetaRef.current.currentMode = opts?.mode ?? providerMetaRef.current.currentMode ?? "auth";
    setProvider(prov);
  ...
  const wrapWithAccountsShim = useCallback((prov: EIP1193Provider, address: string): EIP1193Provider => {
  ...
  const internalInitializeProvider = useCallback(async () => {
    ...
    try {
      if (!providerMetaRef.current.authAdapter) providerMetaRef.current.authAdapter = createAuthAdapter();
      ...
      try {
        let rawChainId: any;
        if (typeof eip1193.request === "function") rawChainId = await eip1193.request({ method: "eth_chainId" });
        if (!rawChainId) rawChainId = (eip1193 as any)?.chainConfig?.chainId;
        const parsed = typeof rawChainId === "string" && rawChainId.startsWith("0x") ? parseInt(rawChainId, 16) : Number(rawChainId);
        if (!Number.isNaN(parsed)) setChainId(parsed);
      } catch (e) {
        log.warn("provider:init:chainId:error", e);
      }
      const accounts = await fetchAccounts(eip1193);
      if (!accounts || accounts.length === 0) scheduleValidation("initial-accounts-empty");
  ...
  const attemptReinitializeProvider = useCallback(async (reason: string) => {
    const now = Date.now();
  ...
  const validateAndMaybeReinit = useCallback(async (reason: string) => {
    if (providerMetaRef.current.providerValidationInFlight) return providerMetaRef.current.providerValidationInFlight;
    const run = (async () => {
      const now = Date.now();
      if (now - providerMetaRef.current.lastValidationTs < validationThrottleMs) return;
    ...
    await run.finally(() => { providerMetaRef.current.providerValidationInFlight = null; });
  }, [attemptReinitializeProvider, fetchAccounts, log, onSessionExpired, provider, providerStatus, validationThrottleMs]);

  // Debounced scheduling to coalesce rapid triggers
  const scheduleValidation = useCallback((reason: string) => {
  ...
  const ensureProvider = useCallback(async () => {
    if (provider || providerStatus === "ready") return;
    if (providerMetaRef.current.providerInitInFlight) return providerMetaRef.current.providerInitInFlight;
    const init = internalInitializeProvider();
    providerMetaRef.current.providerInitInFlight = init;
    await init.finally(() => { providerMetaRef.current.providerInitInFlight = null; });
  }, [internalInitializeProvider, provider, providerStatus]);

  const ensureFreshProvider = useCallback(async () => {
    // If a signing provider became available after init, prefer adopting it
    const override = getSigningProvider();
    // Prefer override only if we are not locked to local mode
    if (override && typeof (override as any).request === "function" && provider !== (override as any)) {
      if (providerMetaRef.current.currentMode !== "local") {
  ...
  useEffect(() => {
    ...
    providerMetaRef.current.healthInterval = setInterval(() => {
      if (AppState.currentState === "active") scheduleValidation("health-interval");
    }, healthIntervalMs);
    return () => { if (providerMetaRef.current.healthInterval) clearInterval(providerMetaRef.current.healthInterval); };
  ...
  const adoptSigningProviderIfAvailable = useCallback(async (): Promise<boolean> => {
    const override = getSigningProvider();
  ...
  return useMemo(() => ({
    ...
    resetProviderState,
  }), [adoptProvider, chainId, ensureFreshProvider, ensureProvider, provider, providerStatus, resetProviderState, validateAndMaybeReinit]);
}
