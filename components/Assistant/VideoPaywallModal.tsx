...
import { getDHBPrice, type AIVideoModel } from '../../services/ai.service';
import { DHB_ADDRESSESS } from '../../config/constants';
import { toastError } from '../../libs';
import { createLogger } from '../../libs/logger';

const log = createLogger('VideoPaywallModal');
...
const VIDEO_MODELS: VideoModel[] = [
  { id: 'kling-2.6-pro', name: 'Kling 2.6 Pro', description: 'Top-tier cinematic with native audio', baseCostUsd: 1.1, hasAudio: true },
  { id: 'luma-ray2', name: 'Luma Ray 2', description: 'Photorealistic, dreamy aesthetic (720p)', baseCostUsd: 0.65 },
  { id: 'runway-gen4', name: 'Runway Gen-4 Turbo', description: 'Animate images (requires image)', baseCostUsd: 0.5 },
  { id: 'minimax-video', name: 'Minimax Video-01', description: 'Fast generation, great quality', baseCostUsd: 0.22 },
];

export interface VideoPaywallModalProps {
  visible: boolean;
  onClose: () => void;
  onConfirm: (model: AIVideoModel) => void;
}
...
const VideoPaywallModalComponent: React.FC<VideoPaywallModalProps> = ({
  visible,
  onClose,
  onConfirm,
}) => {
  ...
  const [selectedModel, setSelectedModel] = useState<AIVideoModel>('kling-2.6-pro');
  ...
  const dropdownData = useMemo(() => VIDEO_MODELS.map((m) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setPaying(true);
    log.debug('Starting video payment:', { model: selectedModel, costInDHB, account });
    try {
          ...
          <ScrollView style={{ flexGrow: 0 }} showsVerticalScrollIndicator={false} bounces={false}>
              ...
              renderItem={(item) => (
                <View style={s.dropdownItem}>
                  <View style={s.dropdownLeft}>
                    <Icon name="Rocket" size={14} color="#A6A9AC" />
                    <View>
                      <View style={s.dropdownNameRow}>
            ...
            <View style={s.costSection}>
              <View style={s.costLine}>
                <Text style={s.costLabel}>Video Cost</Text>
