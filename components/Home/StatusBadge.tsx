...
const StatusBadge: React.FC<BadgeProps> = ({ status, className }) => {
  ...
  return (
    <View className={`absolute left-2 top-2 rounded-md px-2 py-0.5 ${style.container} ${className || ''}`}> 
      <Text className="text-white text-[10px] font-semibold uppercase tracking-wide">{style.label}</Text>
    </View>
  );
};
