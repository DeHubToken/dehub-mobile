...
const TopCommentPreview: React.FC<TopCommentPreviewProps> = ({ comment, total, onOpen }) => {
  const initial = useMemo(() => (comment.user?.[0] || '?').toUpperCase(), [comment.user]);
  ...
  return (
    <TouchableOpacity onPress={onOpen} activeOpacity={0.8} className="mt-4 rounded-2xl border border-theme-neutrals-800 p-3">
      <View className="flex-row items-center justify-between px-1">
        <Text className="text-theme-neutrals-100 text-xs font-semibold">
          Comments <Text className="text-theme-neutrals-500 font-normal">{formatCompactNumber(total)}</Text>
        </Text>
      </View>
