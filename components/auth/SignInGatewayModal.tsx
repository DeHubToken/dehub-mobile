...
const SignInGatewayModal: React.FC<SignInGatewayModalProps> = ({
      ...
      switch (outcome.kind) {
        ...
        case "wallet-setup":
          ...
          if (takeWalletSetupIntent() === "migrate" && outcome.request.mode === "create") {
            setInlineError(t("legacy.noOldAccountFound"));
          }
  ...
  const handleTelegramLogin = useCallback(async () => {
    ...
    try {
      ...
      console.error("[SignInGatewayModal] Telegram login error", e);
      // Closing Telegram's consent screen is a choice, not a failure — do not
      // put a red notice under the sheet for it.
