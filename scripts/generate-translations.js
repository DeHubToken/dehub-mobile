...
// i18n/index.ts is hand-maintained (RTL, plural fill, boot wait); only the
// asset map it reads locales through is generated.
function generateMobileConfig() {
  const configPath = path.join(MOBILE_I18N_DIR, 'localeAssets.ts');

  const loaderEntries = ALL_LOCALES
    .filter((l) => l.code !== 'en')
    .map((l) => `  ${l.code}: () => require('./locale-assets/${l.code}.i18n'),`)
    .join('\n');
 ...
 *
 * Generated by scripts/generate-translations.js.
 */
...
`;
  ...
  console.log(`\nGenerated locale asset map: ${configPath}`);
}
