/**
 * ChatScreen — full-featured Instagram / WhatsApp–style 1-on-1 DM screen.
 *
 * Features:
 * - CDN-only media (no local storage layer)
 * - Send text, GIF, image, video, voice notes
 * - Edit & delete own messages
 * - Forward messages to other conversations
 * - Read receipts, download receipts
 * - Typing indicator
 * - Optimistic messages with reconciliation
 * - DM fee / payment flow
 * - Block / unblock via ChatMenu
 * - Keyboard-aware with jump-to-bottom button
 * - Floating MessageContextMenu on long-press
 */
import React, {
  useCallback,
  useEffect,
...
import MessageBubbleNew from "../components/DM/MessageBubbleNew";
import ChatInputBar, {
...
const ChatScreen: React.FC<ChatScreenProps> = ({ route }) => {
  ...
  // ── Web3 hooks for per-message fee payment ─────────────────────────────
  ...
  // Manual keyboard lift (same pattern as FeedDetailScreen / CommentsBottomSheet)
  ...
  // ─── Fetch target account & DM policy ──────────────────────────────────
  ...
  // ─── Fetch own DM status (creator features) ─────────────────────────
  ...
  /**
   * Execute the on-chain DHB payment for per-message fee.
   * Handles ERC-20 approval + StreamController.sendTip() in one go.
  ...
  // ─── Inject send queue dependencies ──────────────────────────────────
  ...
  // ─── Send handlers (delegate to background queue) ──────────────────────
  ...
  // ─── Reconcile optimistic when server confirms ────────────────────────
