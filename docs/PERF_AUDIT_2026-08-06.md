...
**Date:** 2026-08-06
**Audited tree:** `origin/main` @ `583eefe`. Findings were first surfaced against the in-flight
`agent/mobile-web-ui-parity` working tree and then re-verified against main — all of them are
present on main.
**Predecessor:** `docs/PERF_AUDIT_2026-07-25.md`
**Method:** static only — `node_modules` is present but there is no Node runtime on the audit
machine, so nothing below is a measured number. Counts, config values and file sizes are read
from source.

> **Closed by the branch that adds this doc** (`perf/bundle-and-test-wins`): §2.6 (unstable
> `renderItem`), §2.7 (release console stripping), part of §2.5 (`Avatar` and `FeedImageGallery`
> moved to `SmartImage`), and all of §3 except T5. Everything else below is still open.
